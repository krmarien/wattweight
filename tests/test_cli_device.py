...
@patch("wattweight.cli.device.DeviceManager")
def test_list_devices(mock_manager, device_command: DeviceCommand, logger: Logger):
...
def test_modify_device_not_found(
    ...
    with patch("wattweight.cli.base.get_logger", return_value=logger):
        mock_instance = mock_manager.return_value
        mock_instance.update_device.side_effect = DeviceNotFoundError(
            "Device not found"
        )
        args = argparse.Namespace(
            device_action="modify",
...
def test_remove_device_not_found(
    ...
    with patch("wattweight.cli.base.get_logger", return_value=logger):
        mock_instance = mock_manager.return_value
        mock_instance.delete_device.side_effect = DeviceNotFoundError(
            "Device not found"
        )
        args = argparse.Namespace(device_action="remove", identifier="test")
        result = device_command.execute(args)

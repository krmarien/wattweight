...
def test_register():
    ...
    db_parser = MagicMock()
    subparsers.add_parser.return_value = db_parser
...
def test_execute_no_action(upgrade_command: UpgradeCommand, logger: Logger):
    ...
    args = argparse.Namespace(db_action=None)
...
@patch("wattweight.cli.upgrade.MigrationCore")
def test_upgrade_database_success(
    mock_migration_core, upgrade_command: UpgradeCommand, logger: Logger
):
    """Test upgrading the database successfully."""
    args = argparse.Namespace(db_action="upgrade")
    result = upgrade_command.execute(args)
    assert result == 0
    mock_migration_core.return_value.upgrade.assert_called_once()
    logger.info.assert_any_call("Starting database upgrade...")
    logger.info.assert_any_call("Database upgraded successfully")
...
def test_upgrade_database_runtime_error(
    ...
    mock_migration_core.return_value.upgrade.side_effect = RuntimeError(
        "Upgrade failed"
    )
    args = argparse.Namespace(db_action="upgrade")
    result = upgrade_command.execute(args)
...
def test_upgrade_database_exception(
    ...
    mock_migration_core.return_value.upgrade.side_effect = Exception("Random error")
    args = argparse.Namespace(db_action="upgrade")
    result = upgrade_command.execute(args)
    assert result == 1
...
def test_create_migration_success(
    ...
    result = upgrade_command.execute(args)
    assert result == 0
    mock_migration_core.return_value.create_migration.assert_called_with(
        "New migration"
    )
    logger.info.assert_any_call("Creating migration: New migration")
    logger.info.assert_any_call("Migration created successfully")
...
def test_create_migration_runtime_error(
    ...
    mock_migration_core.return_value.create_migration.side_effect = RuntimeError(
        "Migration failed"
    )
    args = argparse.Namespace(db_action="migrate", message="New migration")
    result = upgrade_command.execute(args)
...
def test_create_migration_exception(
    ...
    mock_migration_core.return_value.create_migration.side_effect = Exception(
        "Random error"
    )
    args = argparse.Namespace(db_action="migrate", message="New migration")
    result = upgrade_command.execute(args)
